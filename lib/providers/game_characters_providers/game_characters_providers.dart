...
@riverpod
Stream<List<GameCharacter>> gameCharacters(GameCharactersRef ref) async* {
  final isar = await ref.watch(isarControllerProvider.future);
  yield (await isar.gameCharacters.where().findAll());
  yield* isar.gameCharacters.where().watch();
}
